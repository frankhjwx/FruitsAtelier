...
internal static class DistanceSnapPresetTests
{
    ...
    public static void Dialog()
    {
        foreach (string language in new[] { "en", "zh-CN" })
        {
            ...
            double expected = Math.Round((range.WalkLimit + range.DashLimit) / 2 / unit, 3);
            Check(ui.Canvas.Texts.Any(t => t.Value == Strings.Get("ds.ratio", expected)), "Slider and numeric field disagree.");
            ...
            Check(ui.Canvas.Texts.Any(t => t.Value == Strings.Get("ds.ratio", expected)), "Cancelled drag did not restore value.");
            var value = ui.Canvas.Texts.Single(t => t.Value == Strings.Get("ds.ratio", expected));
