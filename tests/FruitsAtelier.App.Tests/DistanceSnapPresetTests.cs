...
internal static class DistanceSnapPresetTests
{
    ...
    public static void Dialog()
    {
        foreach (string language in new[] { "en", "zh-CN" })
        {
            ...
            Check(ui.Canvas.Texts.Any(t => t.Value == Strings.Get("ds.bpm", 60000 / beatLength)), "Preview BPM did not use the editor timing.");
            ...
            Check(ui.Canvas.Lines.Count(l => l.X1 == preview.X && l.X2 == preview.Right && l.Y1 == l.Y2) == 16, "Quarter snap did not show exactly sixteen lines.");
            ...
            ui.ClickText(Strings.Get("ds.bpm", 60000 / beatLength)); ui.Type("0"); ui.Key(13);
            Check(ui.View.IsEditingText && ui.Canvas.Texts.Any(t => t.Value == Strings.Get("ds.invalidBpm")), "Invalid BPM accepted.");
            ui.Key('A', ctrl: true); ui.Type("240"); ui.Key(9); ui.Type("1/8"); ui.Key(13);
            Check(ui.Canvas.Lines.Count(l => l.X1 == preview.X && l.X2 == preview.Right && l.Y1 == l.Y2) == 32, "Preview Snap did not change the four-beat grid.");
            Check(ui.View.SnapDivisor == 4 && baseline.ContentEquals(ui.View.Document), "Preview settings changed editor settings or content.");
            ui.ClickText(Strings.Get("ds.snap", 8)); ui.Type("10"); ui.Key(13);
            Check(ui.View.IsEditingText && ui.Canvas.Texts.Any(t => t.Value == Strings.Get("ds.invalidSnap")), "Unsupported snap accepted.");
            ui.Key(27);
            ...
            Check(ui.View.DistanceSnapPreviewFruits.Count == 0 && ui.Canvas.Texts.Any(t => t.Value == Strings.Get("ds.snap", 4)), "Reopening did not reset the preview to editor settings.");
            pointer = ui.View.DistanceSnapPointerBounds.Last();
            ui.View.PointerDown(pointer.X + 8, pointer.Y + 8, 2, false, false); ui.Paint();
