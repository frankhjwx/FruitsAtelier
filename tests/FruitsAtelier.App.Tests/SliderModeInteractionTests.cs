...
internal static class SliderModeInteractionTests
{
    public static void LegacyDraftAndMixedModes()
    {
        ...
        Right(ui, 1500, 200);
        Check(ui.View.ActiveTool == "Slider", "Right-click did not finish legacy drawing.");
    ...
    public static void SwitchArAndLocalPenEdit()
    {
        ...
        ui.View.Wheel(p.X, p.Y, 120 * 18, true, true); ui.Paint();
    ...
    public static void LazerPlacementAndSelection()
    {
        ...
        Check(ui.View.ActiveTool == "Slider", "Completed slider was not selected.");
        var before = ui.View.Document.DeepClone();
        // Selection alone enables control editing; entering B first is unnecessary.
        ui.SelectTrack(ui.View.Document.Tracks[0].Id);
        ...
        Check(ui.View.SelectedAnchorIds.Count == 1, "Ctrl-click did not target the pointed anchor.");
        var target = Screen(ui, 1500, 210);
        ui.View.PointerDown(p.X, p.Y, 0, false, false);
        ui.View.PointerMove(target.X, target.Y, false, false); ui.View.PointerUp(target.X, target.Y, 0); ui.Paint();
        Near(200, ui.View.Document.Tracks[0].Nodes[0].X);
