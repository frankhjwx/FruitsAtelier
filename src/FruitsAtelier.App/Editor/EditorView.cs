...
public sealed partial class EditorView
{
    ...
    public bool WantsCapture => textSelecting || songDrag >= 0 || dsSnapDragging || dsSliderDrag >= 0 || distanceDragging || volumeDrag >= 0 || drag != DragKind.None || libraryPointerActive || tabPointer || streamSnapDragging || SliderHoldNeedsRedraw || sliderHoldConsumed;
    ...
    {
        folder = UpgradeSkinFolder(folder);
        if (CatchSkin.TryLoad(folder, out var loaded, out string message, defaultSkin, allowEmpty: true))
        { skin = loaded; RefreshSkinHitsounds(); }
    ...
    private enum DragKind { None, PlaybackLine, Objects, Anchor, HandleIn, HandleOut, DraftHandle, BananaStart, BananaEnd, Pan, Timeline, Break, Marquee, SnapDivisor, CanvasZoom, LegacyControl, TimelineTail, PreviewResize }
