// Adapted from NAudio v2.2.1, commit b5d5ff83fd378f046398891fe5cd99426ce44732.
// MIT license: Audio/Licenses/NAudio.txt. Playback logic retained; diagnostic counters added.
#nullable disable
using NAudio.Wave;
using System;
...
namespace FruitsAtelier.App.Audio
{
    ...
    /// </summary>
    internal class DiagnosticWasapiOut : IWavePlayer, IWavePosition
    {
        ...
        private long wakeups, waitTimeouts, emptyBufferObservations, submittedFrames;
        private int minimumPadding = int.MaxValue, lastPadding;
        private double lastWakeMs, maximumWakeGapMs, maximumFillMs;
        internal object Snapshot() => new {
            wakeups = Interlocked.Read(ref wakeups), waitTimeouts = Interlocked.Read(ref waitTimeouts),
            emptyBufferObservations = Interlocked.Read(ref emptyBufferObservations),
            submittedFrames = Interlocked.Read(ref submittedFrames),
            minimumPaddingFrames = minimumPadding == int.MaxValue ? (int?)null : minimumPadding,
            lastPaddingFrames = Volatile.Read(ref lastPadding),
            maximumWakeGapMs = Volatile.Read(ref maximumWakeGapMs), maximumFillMs = Volatile.Read(ref maximumFillMs)
        };
        internal object Configuration => new { deviceName = mmDevice.FriendlyName, bufferFrames = audioClient.BufferSize,
            bufferMs = audioClient.BufferSize * 1000d / OutputWaveFormat.SampleRate,
            streamLatencyMs = audioClient.StreamLatency / 10000d,
            defaultPeriodMs = audioClient.DefaultDevicePeriod / 10000d,
            eventWaitTimeoutMs = 3 * latencyMilliseconds, format = OutputWaveFormat.ToString() };
        ...
        public DiagnosticWasapiOut(AudioClientShareMode shareMode, int latency) :
        ...
        /// <param name="latency">Desired latency in milliseconds</param>
        public DiagnosticWasapiOut(AudioClientShareMode shareMode, bool useEventSync, int latency) :
        ...
        /// <param name="useEventSync">true if sync is done with event. false use sleep.</param>
        /// <param name="latency">Desired latency in milliseconds</param>
        public DiagnosticWasapiOut(MMDevice device, AudioClientShareMode shareMode, bool useEventSync, int latency)
        {
        ...
        private void PlayThread()
        {
            ...
            try
            {
                ...
                while (playbackState != PlaybackState.Stopped)
                {
                    ...
                    if (isUsingEventSync)
                    {
                        if (WaitHandle.WaitAny(waitHandles, 3 * latencyMilliseconds, false) == WaitHandle.WaitTimeout)
                    ...
                    double wakeMs = AudioDiagnosticLog.NowMs;
                    if (lastWakeMs != 0) Volatile.Write(ref maximumWakeGapMs, Math.Max(maximumWakeGapMs, wakeMs - lastWakeMs));
                    lastWakeMs = wakeMs;
                    ...
                    if (playbackState == PlaybackState.Playing)
                    {
                        ...
                        Volatile.Write(ref lastPadding, numFramesPadding);
                        minimumPadding = Math.Min(minimumPadding, numFramesPadding);
                        if (numFramesPadding == 0) Interlocked.Increment(ref emptyBufferObservations);
        ...
        private bool FillBuffer(IWaveProvider playbackProvider, int frameCount)
        {
            ...
            if (this.isUsingEventSync && this.shareMode == AudioClientShareMode.Exclusive)
            {
                if (read < readLength)
                {
                    ...
                    // pass frameCount
                    while (read < readLength) Marshal.WriteByte(buffer, read++, 0);
                }
            ...
            Interlocked.Add(ref submittedFrames, read / bytesPerFrame);
            Volatile.Write(ref maximumFillMs, Math.Max(maximumFillMs, AudioDiagnosticLog.NowMs - beganMs));
            return false;
        }
