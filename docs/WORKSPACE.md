...
Right-click a set card to export all of its Catch difficulties as an `.osz` to a chosen location. A workspace project can also be deleted from this menu after confirmation when none of its source or exported `.osu` files exists in the configured osu! Songs directory. Deleting removes its workspace project directory; it does not delete the source set.
